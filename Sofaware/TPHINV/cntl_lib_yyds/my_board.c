...
//    RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOB, ENABLE);
    ...
    if ((HTIM_CHANNEL & 0x01) == 0x01)
    {
		...
		//板子错误的修正
        HRTIM_TIM_OutputStructure.SetSource = HRTIM_RST1R_CMP4;                               // PWM输出高的触发源：CMP1
        HRTIM_TIM_OutputStructure.ResetSource = HRTIM_RST1R_CMP1;                             // PWM输出低的触发源：CMP4
...
void usart_set(u32 baudrate)
{
	...
	gpio_structure.GPIO_Speed=GPIO_Speed_Level_3;
...
inline void my_pwm_update_gai(float duty, uint32 pwm_chennel)
{
    uint32 PWM_PER_0_5 = 0.5f * PWM_PER;
    HRTIM_SlaveSetCompare(HRTIM1, pwm_chennel, HRTIM_COMPAREUNIT_4, PWM_PER_0_5 - duty * 0.5f * PWM_PER);
    HRTIM_SlaveSetCompare(HRTIM1, pwm_chennel, HRTIM_COMPAREUNIT_1, PWM_PER_0_5 + duty * 0.5f * PWM_PER);
