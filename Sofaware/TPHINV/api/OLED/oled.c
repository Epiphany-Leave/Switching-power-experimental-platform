...
//num:数值(0~4294967295);
void OLED_ShowNum(char x, char y, int num, char len, char size2)
{
    char t, temp;
    char enshow = 0;
...
//显示一个字符号串
void OLED_ShowString(char x, char y, char *chr, char Char_Size)
{
    unsigned char j = 0;
...
void OLED_ShowNumf(unsigned char x, unsigned char y, float num, unsigned char len, unsigned char size)
{
    ...
    OLED_ShowNum(x + size / 2, y, num_h, 2, size);
    OLED_ShowChar(x + size * 3 / 2, y, '.', size);
    OLED_ShowNum_0(x + size * 2, y, num_l, len, size);
}
